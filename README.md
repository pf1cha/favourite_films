...
*   **Основной сценарий:**
    ...
    3.  Система формирует запрос к OMDb API на основе введенного текста.
    4.  Система отправляет запрос и получает ответ от OMDb API.
    5. Система отображает список найденных фильмов/сериалов (название, год, тип) на вкладке.
    6. Система отображает сообщение "Найдено: X результатов."
*   **Расширения:**
...
* Загрузка постера: от 50 КБ до 400 КБ (но постеры никак не хранятся в базе данных (только URL-ссылка на них), поэтому можем не учитывать этот момент для расчета)
...
**Пример запроса:**
```
GET /?apikey=secret_key&s=Inception&type=movie&page=1
```
...
**Метод:** `DELETE /favorites/{user_id}/{imdb_id}`  
**Успешный ответ:**
```json
{
  "success": true
...
**Метод:** `GET /reviews/user/{user_id}`  
**Успешный ответ:**
```json
[
  {
...
- **Время отклика для поиска фильмов через OMDb API (включая загрузку постеров):**  
  Время отклика на выполнение поиска фильмов должно быть **меньше 40 секунд** при стабильном интернет-соединении. 
  Ограничение для поиска большое, так как у нас также происходит загрузка постеров для фильма.

- **Время отклика для загрузки списков (избранное, отзывы):**  
  Загрузка списков избранного и отзывов пользователя должна занимать **менее 1 секунды** при количестве записей до 100.

- **Обработка ошибок сети:**  
  При превышении времени отклика (таймаут) или отсутствии ответа от OMDb API, система должна корректно уведомить пользователя о проблеме в течение **5 секунд**.
...
Также она может выдержать нефункциональные требования. 
Приложение корректно обрабатывает сетевые ошибки и таймауты, что предотвращает перегрузку БД.
...
*  **Backend:**
    ...
    *   pytest, unittest - для написания и выполнения тестов.
